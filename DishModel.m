...
@synthesize dishPriceSmall;
@synthesize dishMemberPriceSmall;
@synthesize unitSmall;
@synthesize dishPriceBig;
@synthesize dishMemberPriceBig;
@synthesize unitBig;
@synthesize zhuChu;
@synthesize zhaoPai;
...
+ (NSMutableArray *) searchDish:(NSString *) searchData {
    ...
    NSString *actSQL = [NSString stringWithFormat:@"SELECT * FROM DISH WHERE DISHNAME LIKE '%%%%%@%%';", searchData];
    /*
    int count = 0;
    ...
    if (sqlite3_open(dbpath, &eMenuDB) == SQLITE_OK)
    {
        ...
        if (sqlite3_prepare_v2(eMenuDB, query_stmt, -1, &statement, NULL) == SQLITE_OK)
        {
            while (sqlite3_step(statement) == SQLITE_ROW)
            {
                ...
                // dishPriceSmall
                NSString *dishPriceSmallField = [[NSString alloc] initWithUTF8String:(const char *)sqlite3_column_text(statement, 8)];
                dishModel.dishMemberPriceSmall = dishPriceSmallField;
                // dishMemberPriceSmall
                NSString *dishMemberPriceSmallField = [[NSString alloc] initWithUTF8String:(const char *)sqlite3_column_text(statement, 9)];
                dishModel.dishMemberPriceSmall = dishMemberPriceSmallField;
                // unitSmall
                NSString *unitSmallField = [[NSString alloc] initWithUTF8String:(const char *)sqlite3_column_text(statement, 10)];
                dishModel.unitSmall = unitSmallField;
                // dishPriceBig
                NSString *dishPriceBigField = [[NSString alloc] initWithUTF8String:(const char *)sqlite3_column_text(statement, 11)];
                dishModel.dishPriceBig = dishPriceBigField;
                // dishMemberPriceBig
                NSString *dishMemberPriceBigField = [[NSString alloc] initWithUTF8String:(const char *)sqlite3_column_text(statement, 12)];
                dishModel.dishMemberPriceBig = dishMemberPriceBigField;
                // unitBig
                NSString *unitBigField = [[NSString alloc] initWithUTF8String:(const char *)sqlite3_column_text(statement, 13)];
                dishModel.unitBig = unitBigField;
                // zhuChu
                NSString *zhuChuField = [[NSString alloc] initWithUTF8String:(const char *)sqlite3_column_text(statement, 14)];
                dishModel.zhuChu = zhuChuField;
                // zhaoPai
                NSString *zhaoPaiField = [[NSString alloc] initWithUTF8String:(const char *)sqlite3_column_text(statement, 15)];
                dishModel.zhaoPai = zhaoPaiField;
                // dishTypeId
                NSString *dishTypeIdField = [[NSString alloc] initWithUTF8String:(const char *)sqlite3_column_text(statement, 16)];
...
+ (NSMutableArray *) getAllDish {
    ...
    if (sqlite3_open(dbpath, &eMenuDB) == SQLITE_OK)
    {
        ...
        NSString *querySQL = @"SELECT d.*, dt.TYPENAME FROM DISH d, DISHTYPE dt WHERE d.DISHTYPEID = dt.TYPEID;";
        const char *query_stmt = [querySQL UTF8String];
        if (sqlite3_prepare_v2(eMenuDB, query_stmt, -1, &statement, NULL) == SQLITE_OK)
        {
            while (sqlite3_step(statement) == SQLITE_ROW)
            {
                ...
                // dishPriceSmall
                NSString *dishPriceSmallField = [[NSString alloc] initWithUTF8String:(const char *)sqlite3_column_text(statement, 8)];
                dishModel.dishPriceSmall = dishPriceSmallField;
                // dishMemberPriceSmall
                NSString *dishMemberPriceSmallField = [[NSString alloc] initWithUTF8String:(const char *)sqlite3_column_text(statement, 9)];
                dishModel.dishMemberPriceSmall = dishMemberPriceSmallField;
                // unitSmall
                ...
                dishModel.dishTypeId = dishTypeIdField;
                // dishTypeName
                NSString *dishTypeNameField = [[NSString alloc] initWithUTF8String:(const char *)sqlite3_column_text(statement, 17)];
...
+ (NSMutableArray *) getDishByType:(NSString *) dishTypeId {
    ...
    if (sqlite3_open(dbpath, &eMenuDB) == SQLITE_OK)
    {
        NSString *querySQL = [NSString stringWithFormat:@"SELECT d.*, dt.TYPENAME FROM DISH d, DISHTYPE dt  WHERE dt.TYPEID = d.DISHTYPEID AND d.DISHTYPEID = \"%@\";", dishTypeId];
        const char *query_stmt = [querySQL UTF8String];
        if (sqlite3_prepare_v2(eMenuDB, query_stmt, -1, &statement, NULL) == SQLITE_OK)
        {
            while (sqlite3_step(statement) == SQLITE_ROW)
